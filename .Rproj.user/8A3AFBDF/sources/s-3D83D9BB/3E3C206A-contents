...
# UV are SBS 7a, 7b, 7c, and 7d
# following Lal et. al.
# SBS background rnbinom(1, mu = 2000, size = 1.5)
# SBS1 rnbinom(1, mu = 200, size = 1.5)
# UV signature rnbinom(1, mu = 200, size = 1.5), or 10% increase rnbinom(1, mu = 220, size = 1.5)

generate_sample_signature <- function(uv_increase=20/100){
  uv_increase <- round(uv_increase*200)
  signature <- round(rnbinom(1, mu = 2000, size = 1.5) * norm_cosmic[,"SBS5"]) +
  round(rnbinom(1, mu = 400, size = 1.5) * norm_cosmic[,"SBS1"]) +
...
affected <- t(sapply(1:3, function(x) generate_sample_signature(uv_increase=20/100),USE.NAMES = T))
rownames(affected) <- paste0("A",1:n_samples_per_quadrant)
non_affected <- t(sapply(1:3, function(x) generate_sample_signature(uv_increase=0/100),USE.NAMES = T))
...
res = nmfLasso(x = total, K = 3, beta = beta, background_signature = norm_cosmic[,"SBS5"], seed = 97,normalize_counts = T)
signatures = res$beta
